<script lang="ts">
  ...
  let { onClick, offColor = "surfacevariant", onColor = "primary", value }: LkSwitchProps = $props();
  let switchIsOn = $state(value ? value : false);
  const setSwitchIsOn = (state: boolean) => switchIsOn = state; 
